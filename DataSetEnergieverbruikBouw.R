...
data <- read.csv2("Energieverbruik_Bouw.csv")

data_raw <- read.csv("Energieverbruik_Bouw.csv", header = FALSE, sep = ";", stringsAsFactors = FALSE)
...
df_filtered_bouw <- data_transposed %>%
  filter(Perioden >= 2019)

View(df_filtered_bouw)
